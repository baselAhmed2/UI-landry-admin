...
import {
  ...
  TrendingUp, Clock, ChevronRight, Search, Check, X
} from "lucide-react";
...
export function FraudMonitoring() {
  ...
  const [checkData, setCheckData] = useState({ userId: "", amount: "", ipAddress: "" });
  const [isChecking, setIsChecking] = useState(false);

  const filtered = alerts.filter((a) => filter === "All" || a.risk === filter || a.status === filter);

  const handleCheckPayment = async () => {
    if (!checkData.userId || !checkData.amount) return;
    setIsChecking(true);
    // Simulate POST /api/fraud/payment
    await new Promise(r => setTimeout(r, 1200));
    toast.success("Payment check completed. No immediate fraud detected.");
    setIsChecking(false);
    setShowCheckModal(false);
    setCheckData({ userId: "", amount: "", ipAddress: "" });
  };
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#1D6076] text-white rounded-xl text-sm font-semibold shadow-lg shadow-[#1D6076]/20 hover:bg-[#164a5c] transition-colors"
        >
          <ShieldAlert size={16} /> Verify Payment
        </button>
      </div>
      ...
      <AnimatePresence>
        {showCheckModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => setShowCheckModal(false)} />
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowCheckModal(false)} className="p-1.5 text-slate-400 hover:bg-slate-100 rounded-lg transition-colors">
                  <X size={18} />
                </button>
              </div>
              <p className="text-sm text-slate-500 mb-6">Manually check a specific payment for fraudulent patterns.</p>
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    onChange={e => setCheckData({...checkData, userId: e.target.value})} 
                    placeholder="e.g. U-1045" 
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-[#1D6076]/20 focus:border-[#1D6076]/40"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-700">Amount (SAR)</label>
                  <input 
                    type="number" 
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-700">IP Address (Optional)</label>
                  <input 
                    type="text" 
                    value={checkData.ipAddress} 
                    onChange={e => setCheckData({...checkData, ipAddress: e.target.value})} 
              ...
              <div className="flex gap-3 justify-end mt-6 pt-4 border-t border-slate-100">
                <button onClick={() => setShowCheckModal(false)} className="px-4 py-2 text-slate-600 font-medium text-sm hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
                <button 
                  onClick={handleCheckPayment}
                  disabled={!checkData.userId || !checkData.amount || isChecking}
                  className="px-4 py-2 bg-[#1D6076] hover:bg-[#164a5c] text-white font-medium text-sm rounded-lg transition-colors shadow-lg shadow-[#1D6076]/20 disabled:opacity-50 flex items-center gap-2"
                >
                  {isChecking ? <Clock size={16} className="animate-spin" /> : <ShieldAlert size={16} />}
                  Run Analysis
                </button>
              </div>
            </motion.div>
