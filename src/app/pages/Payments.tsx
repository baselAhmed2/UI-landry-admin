...
function PlatformCommissionCard() {
  ...
  const commissionDue = totalRevenue * COMMISSION_RATE;

  const handlePayWithKashier = () => {
    setLoading(true);
    // In production: window.location.href = kashierCheckoutUrl
    setTimeout(() => {
      setLoading(false);
      setPaid(true);
    }, 2200);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            sub: `${allPayments.filter((p) => p.status === "Pending").length} orders`,
            icon: Clock,
            accent: false,
          },
          {
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 pt-5 pb-4 border-b border-gray-50">
          <div className="flex gap-1 overflow-x-auto pb-1 sm:pb-0">
            {tabs.map((tab) => {
              const count = tab === "All" ? allPayments.length : allPayments.filter((p) => p.status === tab).length;
              return (
                <button
                  key={tab}
