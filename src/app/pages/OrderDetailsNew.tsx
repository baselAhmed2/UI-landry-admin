...
export function OrderDetailsNew() {
  ...
  const order = (id && orderData[id]) ? orderData[id] : { ...defaultOrder, id: id || "ORD-????" };
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-5">
      ...
      >
        ...
        <div className="mt-4 pt-4 border-t border-gray-50 flex flex-wrap items-center gap-2">
          ...
          {["Processing", "Ready", "Delivered", "Cancelled"].map((s) => (
            <motion.button
              key={s}
              whileTap={{ scale: 0.96 }}
              ...
              style={currentStatus === s ? { backgroundColor: statusConfig[s]?.color } : {}}
            >
              {s}
            </motion.button>
          ))}
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          >
            <div className="px-5 py-4 border-b border-gray-50">
              ...
              <p className="text-xs text-gray-400 mt-0.5">{order.service} · {order.items.reduce((a, i) => a + i.qty, 0)} items total</p>
            </div>
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium text-gray-800">{order.items.reduce((a, i) => a + i.qty, 0)}</span>
              </div>
              <div className="flex justify-between text-sm">
          ...
          >
            ...
            <div className="space-y-2">
              <motion.button
                ...
                disabled={currentStatus === "Ready" || currentStatus === "Delivered" || currentStatus === "Cancelled"}
                className="w-full flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-xl text-white transition-all hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                style={{ backgroundColor: currentStatus === "Ready" ? "#22c55e" : "#1D5B70" }}
              >
                {currentStatus === "Ready" ? <CheckCircle2 className="w-4 h-4" /> : <Truck className="w-4 h-4" />}
                {currentStatus === "Ready" ? "Order is Ready ✓" : "Mark as Ready"}
              </motion.button>
              <motion.button
                whileTap={{ scale: 0.98 }}
                onClick={handleSendSms}
